...
- 由于 `node_modules` 太大，且默认是可以被上传的。所以我们需要创建一个 `.gitignore` 文件，然后就可以忽略上传某个目录或者文件了。
- 步骤如下:
  ```text
  cd reverse_practice
  touch .gitignore
...
- 本地推送至`dockerhub`
  ```nginx
  cd reverse_practice
  docker build -t reverse_practice:latest .
  docker tag reverse_practice:latest lovesdog/reverse_practice:latest
  docker push reverse_practice:latest
  ```
- 从`dockerhub`拉取项目至本地
  - `docker pull lovesdog/reverse_practice:latest`
...
看准网 |  [链接](https://www.kanzhun.com/firm/wage/1HV52g~~/p1.html)  |    |  [kanzhun](kanzhun)
崔佬搭建的练习网站  |  [链接](https://spa2.scrape.center/page/1)  |    |  [scrape_spa2](yuanrenxue/scrape_spa2)
...
酷我音乐  |  [链接](http://www.kuwo.cn/search/list?key=%E4%B8%8B%E5%B1%B1)  |    |  [music_kuwo](music_kuwo)
